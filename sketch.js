...
var Ott=class Ott{
  ...
  ai(){}
  air(){
    if(this.fx>0){this.fx=Math.max(this.fx-air,0)}
    
    if(this.fx<0){this.fx=Math.min(this.fx+air,0)}
    
    if(this.fy>0){this.fy=Math.max(this.fy-air,0)}
    
    if(this.fy<0){this.fy=Math.min(this.fy+air,0)}
  }
  wrap(){
    if(this.x>cwidth){
      this.x=0;
